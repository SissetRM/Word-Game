...
def getWordScore(word, n):
    ...
    if len(word) == n:
        score = sum(points)*len(word) + 50
    else:
        score = sum(points) * len(word)
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:  # As long as there are still letters left in the hand:
        ...
        if word == '.':  # If the input is a single period:
            print("Goodbye! Total score: " + str(score))  # End the game (break out of the loop)
            break
        else:  # Otherwise (the input is not a single period):
            if not isValidWord(word, hand, word_list):  # If the word is not valid:
                ...
                print('"' + word + '" ' + 'earned ' + str(getWordScore(word, n))
                      + ' points. Total: ' + str(score) + ' points.')  # Tell the user how many points the word earned, and the updated total score, in one line followed by a blank line
                print()
                hand = updateHand(hand, word)  # Update the hand
    if calculateHandlen(hand) == 0:
        print('Run out of letters. Total score: '
              + str(score) + ' points.')  # Game is over (user entered a '.' or ran out of letters), so tell user the total score
...
def playGame(wordList):
    ...
    while True:
        n = HAND_SIZE
        startGame = input('Enter "n" to deal a new hand, r to replay the last hand, or e to end game: ')
        if startGame == 'n':
            hand = dealHand(n)
